...
apiController.getData = async (req, res, next) => {
  ...
  // try {
    ...
    if(cuisineType !== undefined) { //check if cuisine type is passed in
      queries += `&cuisineType=${cuisineType[0]}`;
    }
    // adds meal type to the query
    if(mealType !== undefined) { //check if meal dish type is passed in
      queries += `&mealType=${mealType[0]}`;
    }
    if(dishType !== undefined) { //check if dish type is passed in
  ...
  try {
    ...
    } else { // otherwise use the large image size 
